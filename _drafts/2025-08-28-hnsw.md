...
Quando comecei a estudar sobre banco de dados vetoriais, fiquei surpreso ao descobrir que não existe garantia de resultado ótimo. Por exemplo, ao buscar pelos $$ k $$ vetores mais próximos de um vetor $$ v $$, não é garantido que os vetores retornados serão os mais próximos do conjunto.
...
Algo que eu sempre digo: se existisse uma forma trivial de escolher os hiper-parâmetros, eles seriam constantes ou escolhidos automaticamente. Assim como ajustar os parâmetros de uma *random forest*, é importante realmente entender a dinâmica dos parâmetros, para não perder tempo com uma tentativa e erro sem direcionamento.

Eu [implementei o algoritmo](https://github.com/gdarruda/hnsw-python) apenas para fins didáticos, nem tenho certeza de sua corretude, mas é um jeito que me ajuda a realmente entender os problemas. Talvez, eu retome esse assunto de como otimizar as consultas, mas é um assunto que merece um outro post.