...
Se o parâmetro `extend_candidates` for verdadeiro, a seleção inclui os "vizinhos dos vizinhos" na busca. Com esses candidatos extras, a seleção pode criar conexões entre grupos de vetores que poderiam ficar inacessíveis. Se o parâmetro `extend_candidates` for falso, `select_neighbors` e `select_neighbors_heuristic` são o mesmo algoritmo. 
